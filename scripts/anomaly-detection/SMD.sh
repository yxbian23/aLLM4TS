...
if [ ! -d "./logs/anomaly_detection/SMD" ]; then
    mkdir ./logs/anomaly_detection/SMD
fi

if [ ! -d "./checkpoints/anomaly_detection/SMD" ]; then
    mkdir ./checkpoints/anomaly_detection/SMD
fi
...
root_path_name=./dataset/SMD
data_path_name=SMD.csv
data_name=SMD
...
do
    ...
    --checkpoints ./checkpoints/anomaly_detection/SMD \
    > logs/anomaly_detection/SMD/$anomaly_ratio'_'$model_id'_sl'$seq_len'_pl'$pred_len'_llml'$llm_layers'_lr'$lr'_bs'$bs'_percent'$percent'_'$exp_des.log 2>&1 
done
done
done
done
