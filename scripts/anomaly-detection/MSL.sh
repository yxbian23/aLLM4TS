...
if [ ! -d "./logs/anomaly_detection/MSL" ]; then
    mkdir ./logs/anomaly_detection/MSL
fi

if [ ! -d "./checkpoints/anomaly_detection/MSL" ]; then
    mkdir ./checkpoints/anomaly_detection/MSL
fi
...
do
    ...
    --checkpoints ./checkpoints/anomaly_detection/MSL \
    > logs/anomaly_detection/MSL/$anomaly_ratio'_'$model_id'_sl'$seq_len'_pl'$pred_len'_llml'$llm_layers'_lr'$lr'_bs'$bs'_'$exp_des.log 2>&1 
done
done
done
done
