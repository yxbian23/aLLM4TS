...
if [ ! -d "./logs/imputation/ETTh1" ]; then
    mkdir ./logs/imputation/ETTh1
fi

if [ ! -d "./checkpoints/imputation/ETTh1" ]; then
    mkdir ./checkpoints/imputation/ETTh1
fi
...
do
    ...
    --checkpoints ./checkpoints/imputation/ETTh1 \
    > logs/imputation/ETTh1/$mask_rate'_'$model_id'_sl'$seq_len'_pl'$pred_len'_llml'$llm_layers'_lr'$lr'_bs'$bs'_percent'$percent'_'$exp_des.log 2>&1 
done
done
done
done
