...
if [ ! -d "./logs/imputation/ETTh2" ]; then
    mkdir ./logs/imputation/ETTh2
fi

if [ ! -d "./checkpoints/imputation/ETTh2" ]; then
    mkdir ./checkpoints/imputation/ETTh2
fi
...
do
    ...
    --checkpoints ./checkpoints/imputation/ETTh2 \
    > logs/imputation/ETTh2/$mask_rate'_'$model_id'_sl'$seq_len'_pl'$pred_len'_llml'$llm_layers'_lr'$lr'_bs'$bs'_percent'$percent'_'$exp_des.log 2>&1 
done
done
done
done
