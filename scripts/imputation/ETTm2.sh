...
if [ ! -d "./logs/imputation/ETTm2" ]; then
    mkdir ./logs/imputation/ETTm2
fi

if [ ! -d "./checkpoints/imputation/ETTm2" ]; then
    mkdir ./checkpoints/imputation/ETTm2
fi
...
do
    ...
    --checkpoints ./checkpoints/imputation/ETTm2 \
    > logs/imputation/ETTm2/$mask_rate'_'$model_id'_sl'$seq_len'_pl'$pred_len'_llml'$llm_layers'_lr'$lr'_bs'$bs'_percent'$percent'_'$exp_des.log 2>&1 
done
done
done
done
